...
class MSSTdata(data.Dataset):
    def __init__(self, dataroot, modality='joint', test_mode=False, seg=8):
        datapath = os.path.join(dataroot, 'train_joint.pkl')
        if test_mode:
            datapath = os.path.join(dataroot, 'val_joint.pkl')
    ...
    def __getitem__(self, index):
        ...
        sk_data = self.all_data[key]#Tx17x3
        T, V, C = sk_data.shape

        sk_data = np.array([np.transpose(sk_data, (2, 0, 1))])#3XTx17,为了适应后续的变化，这里在外面加一维。变成1x3xTx17
