...
class TSNDataSet(data.Dataset):
    def __init__(self, rgb_root, video_list,
                 num_segments=3, new_length=1, modality='RGB',
                 image_tmpl='img_{:05d}.jpg', transform=None,
                 random_shift=True, test_mode=False,
                 remove_missing=False, dense_sample=False, twice_sample=False):

        self.root_path = rgb_root
        self.all_video_list = open(video_list, 'r')
    ...
    def _parse_list(self):
        self.video_list_info = [x for x in self.all_video_list.readlines()]
        print('video number:%d' % (len(self.video_list_info)))

    def _sample_indices(self, video_name):
        # ts = time.time()

        if self.modality == 'RGB':
            video_name = video_name + '_rgb.avi'
        elif self.modality == 'depth':
            video_name = video_name + '_depth.avi'
        elif self.modality == 'skmap':
    ...
    def __getitem__(self, index):
        video_info = self.video_list_info[index]
        record = video_info.split(' ')[0]
        # check this is a legit video folder
        label = int(record[-3:]) - 1
        if not self.test_mode:
            use_images = self._sample_indices(record) if self.random_shift else self._get_val_indices(record)
        else:
            use_images = self._get_test_indices(record)
