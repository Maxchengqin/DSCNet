...
class TSNDataSet(data.Dataset):
    ...
    def _load_image(self, directory, idx):
        if self.modality in ['RGB', 'motion', 'dense','depth','depth_dense']:
            try:
    ...
    def _sample_indices(self, record):
        ...
        if self.dense_sample:  # i3d dense sample
            ...
            elif record.num_frames > self.num_segments and (record.num_frames+1) > self.new_length:
                # offsets = np.sort(randint(record.num_frames - self.new_length + 1, size=self.num_segments))
                offsets = np.sort(randint(record.num_frames - self.new_length, size=self.num_segments))
            else:
        ...
        for seg_ind in indices:
            ...
            for i in range(self.new_length):
                ...
                if p < record.num_frames:
                    p += 1
        # te_l = time.time()
        # print('读取图片耗时：', te_l - ts_l)
        process_data = self.transform(images)
        # te_p = time.time()
        # print('处理图片耗时：', te_p - te_l)
