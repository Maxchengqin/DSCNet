...
class MSSTdata(data.Dataset):
    ...
    def __getitem__(self, index):
        key = self.all_keys[index] # UAV:#[/data/cq/UAV/ori/xsub1/test/P113S28G10B40H10UC091000LC021000A060R0_10041547.txt'],A后面的label不用减1.
        sk_data = self.all_data[key]##UAV [3,T,17,2] 有置信度，但都是0
        M, T, V, C = sk_data.shape
        if M==1:
            sk_data = np.concatenate((sk_data, np.zeros_like(sk_data)), 0)

        sk_data = np.transpose(sk_data, (3, 0, 1, 2)) #变成Mx3XTx17,为了适应后续的变化，
        ...
        label = int(key.split('/')[-1].split("A")[-1][0:3])
